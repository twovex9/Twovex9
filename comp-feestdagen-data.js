  ...
  async function pushAll(items) {
    ...
    try {
      ...
      if (existingHead.error) {
        console.error("[compFeestdagenDB] pushAll select mislukt:", existingHead.error);
        return;
      }
      ...
      if (items.length) {
        ...
        if (ups.error) console.error("[compFeestdagenDB] upsert mislukt:", ups.error);
      }
      if (toDelete.length) {
        var del = await global.besaSupabase.from(TABLE).delete().in("id", toDelete);
        if (del.error) console.error("[compFeestdagenDB] delete mislukt:", del.error);
      }
    } catch (err) {
      console.error("[compFeestdagenDB] pushAll error:", err);
    }
  }
