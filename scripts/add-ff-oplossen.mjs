#!/usr/bin/env node
/**
 * add-ff-oplossen.mjs — voeg <script src="ff-oplossen.js" defer> toe aan
 * alle app-HTML-pagina's, direct NA ff-sync-reporter.js.
 *
 * ff-oplossen.js levert window.ffOplossen (de "Oplossen →"-knop + popover +
 ...
 * ff-sync-reporter.js (vóór de overige data-lagen/page-scripts) is veilig.
 *
 * Idempotent: slaat pagina's over die het al hebben. Slaat pagina's zonder
 * ff-sync-reporter.js over (publieke/portal-fragmenten die de bel niet tonen).
 *
 *   node scripts/add-ff-oplossen.mjs            # write
 *   node scripts/add-ff-oplossen.mjs --check    # dry-run
 */
...
const REPORTER_RE = /(<script\s+src="ff-sync-reporter\.js[^"]*"[^>]*><\/script>)/i;
...
for (const file of readdirSync(projectRoot)) {
  ...
  if (html.includes("ff-oplossen.js")) { skipped++; continue; }
  const m = html.match(REPORTER_RE);
  if (!m) { noAnchor++; continue; }
  const version = (m[1].match(/\?v=([^"]+)/) || [, "918f1ad"])[1];
  // NIET defer: ff-oplossen.js doet bij load geen DOM-toegang (zet enkel
  // window.ffOplossen), en sommige pagina's hebben een NIET-deferred page-script
  // dat anders eerder draait dan een deferred ff-oplossen.js → window.ffOplossen
  // undefined bij eerste render. Non-defer garandeert beschikbaarheid voor elk
  // page-script, ongeacht hun defer-status.
  const tag = `\n  <script src="ff-oplossen.js?v=${version}"></script>`;
