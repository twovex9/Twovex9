  ...
  async function pushAll(items) {
    ...
    try {
      ...
      if (existingHead.error) {
        console.error("[compBerekeningenDB] pushAll select mislukt:", existingHead.error);
        return;
      }
      ...
      if (items.length) {
        ...
        if (ups.error) console.error("[compBerekeningenDB] upsert mislukt:", ups.error);
      }
      if (toDelete.length) {
        var del = await global.besaSupabase.from(TABLE).delete().in("id", toDelete);
        if (del.error) console.error("[compBerekeningenDB] delete mislukt:", del.error);
      }
    } catch (err) {
      console.error("[compBerekeningenDB] pushAll error:", err);
    }
  }
