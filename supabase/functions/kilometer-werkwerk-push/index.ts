// Supabase Edge Function: kilometer-werkwerk-push
// Gerichte Web Push voor werk-werk kilometer-meldingen (clientvervoer).
// Server-side getriggerd door public.kilometer_werkwerk_notify() via pg_net met
// een gedeelde cron-secret (GEEN user-JWT -> verify_jwt = false). Pusht per
// notificatie naar de push_subscriptions van DIE ene gebruiker. Deep-link:
// coordinator-melding -> /kilometers/goedkeuren, medewerker -> /kilometers.
// Patroon 1-op-1 van taken-herinnering-push.
...
const KM_PUSH_TYPES = [
  "km_werkwerk_aangevraagd",
  "km_werkwerk_goedgekeurd",
  "km_werkwerk_afgewezen",
];
...
serve(async (req: Request) => {
  ...
  // 1. Cron-secret check (geen user-JWT; draait alleen server-side).
  ...
  // 3. De betreffende werk-werk-notificaties.
  ...
  for (const n of notifs) {
    ...
    const deepUrl = n.type === "km_werkwerk_aangevraagd" ? "/kilometers/goedkeuren" : "/kilometers";
    const payload = JSON.stringify({ title: n.title, body: n.body || "", url: deepUrl, tag: "km-werkwerk-" + (n.related_entity_id || n.id) });
    for (const s of subs) {
