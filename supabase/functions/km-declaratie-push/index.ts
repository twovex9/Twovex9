// Supabase Edge Function: km-declaratie-push
// Gerichte Web Push voor de melding "kilometerdeclaratie staat klaar ter
// goedkeuring". Wordt server-side getriggerd door public.km_genereer_vorige_maand()
// (pg_cron, 1e van de maand) via pg_net met een gedeelde cron-secret
// (km_push_cron_secret) -> verify_jwt = false. De in-app notificatie bestaat dan
// al; dit is enkel het telefoon-signaal er bovenop. De url is een deep-link naar
// de kilometer-declaratie zodat tikken de juiste maand opent.
// Patroon gekopieerd van `taken-herinnering-push`.
...
const KM_PUSH_TYPES = ["km_declaratie_klaar"];
...
serve(async (req: Request) => {
  ...
  const secResp = await supa.from("private_app_config").select("waarde").eq("sleutel", "km_push_cron_secret").maybeSingle();
  ...
  // 3. De betreffende notificaties (alleen km-declaratie-types).
  const nResp = await supa.from("notifications").select("id, user_id, title, body, related_entity_id, type").in("id", ids).in("type", KM_PUSH_TYPES);
  ...
  // 6. Versturen: per notificatie naar de subs van die ene gebruiker. De url is
  //    een deep-link naar de kilometer-declaratie zodat tikken de juiste maand opent.
  let verstuurd = 0;
  let mislukt = 0;
  for (const n of notifs) {
    const subs = subsByUser.get(n.user_id) || [];
    if (subs.length === 0) continue;
    const deepUrl = n.related_entity_id ? `/kilometers?decl=${n.related_entity_id}` : "/kilometers";
    const payload = JSON.stringify({ title: n.title, body: n.body || "", url: deepUrl, tag: "km-" + (n.related_entity_id || n.id) });
    for (const s of subs) {
