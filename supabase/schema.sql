...
-- =============================================================================
-- Stage 9f: incidenten — uitbreiding voor uitgebreid meldingsformulier
-- (Ouders/vertegenwoordigers, Feedback, Notificaties, Bijlagen)
-- =============================================================================
-- Voegt 5 nieuwe kolommen toe aan public.incidenten:
--   - ouders_geinformeerd        : ja/nee/null (3 staten — null = nog niet beantwoord)
--   - wil_gebeld_worden          : checkbox in Feedback-sectie
--   - impact_op_zorgverlener     : vrije tekst over persoonlijke impact
--   - notificeer_team            : toggle "Mijn team informeren"
--   - notificeer_medewerker_ids  : uuid[] voor "Specifieke medewerkers informeren"
--
-- Plus een aparte tabel + Storage bucket voor bijlagen
-- (foto's, documenten — PDF/DOC/XLS/CSV/TXT/MSG/JPG/PNG):
--   - public.incident_documenten  (metadata + storage_path)
--   - storage bucket "incident-documenten"
-- Patroon spiegelt medewerker_documenten / client_documents.
-- =============================================================================
alter table public.incidenten
  add column if not exists ouders_geinformeerd boolean,
  add column if not exists wil_gebeld_worden boolean not null default false,
  add column if not exists impact_op_zorgverlener text not null default '',
  add column if not exists notificeer_team boolean not null default false,
  add column if not exists notificeer_medewerker_ids uuid[] not null default '{}'::uuid[];

create table if not exists public.incident_documenten (
  id text primary key,
  incident_id uuid not null references public.incidenten(id) on delete cascade,
  naam text not null default '',
  uploaddatum timestamptz not null default now(),
  laatst_gewijzigd timestamptz not null default now(),
  archived boolean not null default false,
...
create index if not exists incident_documenten_incident_id_idx
  on public.incident_documenten (incident_id);
create index if not exists incident_documenten_archived_idx
  on public.incident_documenten (archived);
create index if not exists incident_documenten_storage_path_idx
  on public.incident_documenten (storage_path)
  where storage_path is not null;

drop trigger if exists trg_incident_documenten_set_modified on public.incident_documenten;
create trigger trg_incident_documenten_set_modified
  before update on public.incident_documenten
  for each row execute function public.set_laatst_gewijzigd();

alter table public.incident_documenten enable row level security;

drop policy if exists "auth kan incident_documenten lezen" on public.incident_documenten;
create policy "auth kan incident_documenten lezen"
  on public.incident_documenten for select to authenticated using (true);

drop policy if exists "auth kan incident_documenten toevoegen" on public.incident_documenten;
create policy "auth kan incident_documenten toevoegen"
  on public.incident_documenten for insert to authenticated with check (true);

drop policy if exists "auth kan incident_documenten bewerken" on public.incident_documenten;
create policy "auth kan incident_documenten bewerken"
  on public.incident_documenten for update to authenticated using (true) with check (true);

drop policy if exists "auth kan incident_documenten verwijderen" on public.incident_documenten;
create policy "auth kan incident_documenten verwijderen"
  on public.incident_documenten for delete to authenticated using (true);

insert into storage.buckets (id, name, public)
values ('incident-documenten', 'incident-documenten', true)
on conflict (id) do nothing;

drop policy if exists "auth kan incident-documenten lezen" on storage.objects;
create policy "auth kan incident-documenten lezen"
  on storage.objects for select to authenticated
  using (bucket_id = 'incident-documenten');

drop policy if exists "auth kan incident-documenten uploaden" on storage.objects;
create policy "auth kan incident-documenten uploaden"
  on storage.objects for insert to authenticated
  with check (bucket_id = 'incident-documenten');

drop policy if exists "auth kan incident-documenten bewerken" on storage.objects;
create policy "auth kan incident-documenten bewerken"
  on storage.objects for update to authenticated
  using (bucket_id = 'incident-documenten')
  with check (bucket_id = 'incident-documenten');

drop policy if exists "auth kan incident-documenten verwijderen" on storage.objects;
