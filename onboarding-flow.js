...
(function (global) {
  ...
  var BLOCKED_FLAG = "ff-onboarding-blocked";

  async function getProfileFlags() {
    if (!global.ffSupabase) return null;
    var user = await global.ffSupabase.auth.getUser();
  ...
  function buildOverlay() {
    ...
    overlay.id = "ff-onboarding-modal";
  ...
  async function showPasswordModal() {
    ...
    dialog.innerHTML = '' +
      ...
      '</div>' +
      '<form id="ff-onb-pw-form" style="padding:22px 26px;display:flex;flex-direction:column;gap:14px;">' +
        '<label style="display:flex;flex-direction:column;gap:4px;font-size:13px;">' +
          'Nieuw wachtwoord' +
          '<input type="password" id="ff-onb-pw1" autocomplete="new-password" required minlength="8" style="padding:8px 10px;border:1px solid var(--line);border-radius:var(--r-sm);font:inherit;">' +
        '</label>' +
        '<label style="display:flex;flex-direction:column;gap:4px;font-size:13px;">' +
          'Bevestig wachtwoord' +
          '<input type="password" id="ff-onb-pw2" autocomplete="new-password" required minlength="8" style="padding:8px 10px;border:1px solid var(--line);border-radius:var(--r-sm);font:inherit;">' +
        '</label>' +
        '<p style="margin:0;color:var(--text-muted,#666);font-size:12px;">Minimaal 8 tekens, 1 cijfer, 1 hoofdletter.</p>' +
        '<p id="ff-onb-pw-err" style="margin:0;color:var(--red);font-size:13px;display:none;"></p>' +
        '<div style="display:flex;justify-content:flex-end;margin-top:6px;">' +
          '<button type="submit" class="btn-primary" id="ff-onb-pw-submit">Wachtwoord opslaan</button>' +
        '</div>' +
      '</form>';
    document.body.appendChild(built.overlay);

    return new Promise(function (resolve, reject) {
      document.getElementById("ff-onb-pw-form").addEventListener("submit", async function (e) {
        e.preventDefault();
        var pw1 = document.getElementById("ff-onb-pw1").value;
        var pw2 = document.getElementById("ff-onb-pw2").value;
        var errEl = document.getElementById("ff-onb-pw-err");
        var submitBtn = document.getElementById("ff-onb-pw-submit");
        ...
        try {
          var upd = await global.ffSupabase.auth.updateUser({ password: pw1 });
          if (upd.error) throw upd.error;
          var user = await global.ffSupabase.auth.getUser();
          await global.ffSupabase
            .from("profiles")
  ...
  async function show2faModal() {
    ...
    dialog.innerHTML = '' +
      ...
      '<div style="padding:22px 26px;display:flex;flex-direction:column;gap:14px;">' +
        ...
        '<div id="ff-onb-2fa-qr" style="display:flex;justify-content:center;background:#FFFFFF;padding:20px;border-radius:var(--r-md);min-height:240px;align-items:center;border:1px solid var(--line,#e5e7eb);">Bezig met laden…</div>' +
        '<p id="ff-onb-2fa-secret" style="margin:0;text-align:center;font-family:monospace;font-size:11px;color:var(--text-muted);"></p>' +
        '<label style="display:flex;flex-direction:column;gap:4px;font-size:13px;">' +
          'Vul 6-cijferige code in' +
          '<input type="text" id="ff-onb-2fa-code" inputmode="numeric" maxlength="6" pattern="[0-9]{6}" required style="padding:8px 10px;border:1px solid var(--line);border-radius:var(--r-sm);font:inherit;letter-spacing:2px;text-align:center;font-size:18px;">' +
        '</label>' +
        '<p id="ff-onb-2fa-err" style="margin:0;color:var(--red);font-size:13px;display:none;"></p>' +
        '<div style="display:flex;justify-content:flex-end;">' +
          '<button type="button" class="btn-primary" id="ff-onb-2fa-submit">Activeer 2FA</button>' +
        '</div>' +
      '</div>';
    document.body.appendChild(built.overlay);
    ...
    try {
      ...
      var en = await global.ffSupabase.auth.mfa.enroll({
        factorType: "totp",
        issuer: "Future Flow",
      ...
      enrollData = en.data;
      var qrEl = document.getElementById("ff-onb-2fa-qr");
      ...
      var secEl = document.getElementById("ff-onb-2fa-secret");
      secEl.textContent = "Of voer handmatig in: " + enrollData.totp.secret;
    } catch (err) {
      document.getElementById("ff-onb-2fa-qr").textContent = "Fout bij genereren QR: " + (err.message || err);
    }

    return new Promise(function (resolve) {
      document.getElementById("ff-onb-2fa-submit").addEventListener("click", async function () {
        var code = document.getElementById("ff-onb-2fa-code").value.trim();
        var errEl = document.getElementById("ff-onb-2fa-err");
        var btn = document.getElementById("ff-onb-2fa-submit");
        errEl.style.display = "none";
        ...
        try {
          var ch = await global.ffSupabase.auth.mfa.challenge({ factorId: enrollData.id });
